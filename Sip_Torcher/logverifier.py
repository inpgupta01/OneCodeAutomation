...
def usage():
    ...
    my_logger.info('logcheck:Method:<SIP Method[|SIP Method]> Mocc:<Occurence No| last> [Header:<SIP-header>  Hocc:<any|integer> Hstring:<Value of SIP-header>] [SDP:<True|False>] [Hsdp:<SIP-header> Hsdpocc:<last|any|integer> Hsdpstr:<Value of SIP-header>]')
         ...
         if(tmplist[0].lower() == 'hsdpocc'):
            if SDPheaderfound:
               if(re.match(r'\d',tmplist[1])):
                  ...
                  SDPheaderoccfound = True
                  SDPhstringfound = False
               elif re.match('last',tmplist[1].lower()):
                  SDPheadername = SDPheadername + '_' + '0'
                  SDPheaderoccfound = True
                  SDPhstringfound = False
...
def retMsg(logfn):
    ...
    for line in logfnhandle:
        ...
        if(re.match('.{3,4} message received ', line)): 
           #print("========")
           msgRcvd=True
           RecordNow=False
           DBL=True
