...
expected_res_count = {
    ...
    5: {2048: {'actual_1080p_stream_count': 1, 'actual_720p_stream_count': 0, 'actual_360p_stream_count': 3,
               'actual_180p_stream_count': 0},
        1920: {'actual_1080p_stream_count': 0, 'actual_720p_stream_count': 1, 'actual_360p_stream_count': 3,
               'actual_180p_stream_count': 0},
        1024: {'actual_1080p_stream_count': 0, 'actual_720p_stream_count': 1, 'actual_360p_stream_count': 3,
               'actual_180p_stream_count': 0},
        768: {'actual_1080p_stream_count': 0, 'actual_720p_stream_count': 1, 'actual_360p_stream_count': 3,
              'actual_180p_stream_count': 0},
        512: {'actual_1080p_stream_count': 0, 'actual_720p_stream_count': 0, 'actual_360p_stream_count': 4,
              'actual_180p_stream_count': 0}},
...
def max_bit_rate_calculation(json_data, lr):
    ...
    if total_bit_rate_used < lr:
        print(
            "Total Bit Rate used by EP is " + str(total_bit_rate_used) + "kbps < Negotiated Bandwidth  " + str(
                lr) + "kbps")
        return 0
    else:
        print("Total Bit Rate used by EP is " + str(
            total_bit_rate_used) + "kbps exceeds Negotiated Bandwidth  " + str(lr) + "kbps")
        return 1
...
def validate_streams_health(json_data):
    ...
    for i in json_data:
        if i['mediaStream'] == 'videoRx':
            ...
            if i['actualBitRate'] < i['bitRate'] + 10:
                print(
                    "Rx Bit Rate Used " + str(i['actualBitRate']) + " <= Max Bit Rate" +
                    str(i['bitRate']) + " Upto +10 kbps is acceptable RESULT:[PASS]")
            else:
                print(
                ...
                negative += 1
            if i['jitter'] < 20:
                print("Jitter is less than 20 msec RESULT:[PASS]")
            else:
                print("Jitter exceeds 20 msec RESULT:[FAIL]")
                negative += 1
            if i['percentPacketLoss'] == 0:
