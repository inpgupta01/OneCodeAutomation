...
def main_function(arg):
    ...
    print("###########TEST CASE STARTED FOR LR " + str(lr[4]) + "############################\n\n")
    fail_per_lr = 0
    # Connecting calls on GS

    print("#####################Connecting calls on GS############## \n ")
    for ep in final_list:
        print("Endpoint " + str(ep) + " is connecting to VMR " + str(int(vmr[4])) + " at line rate " + str(
            int(lr[4])) + " kbps.")
        calling(ep, int(vmr[4]), int(lr[4]))
        time.sleep(3)
    ...
    if connection_status == 0:
        ...
        print("Not all EPs successfully connected to conference..Check logs for Eps Disconnection cause Result: ["
              "FAIL] ")
        print("##########Test Case failed for LR " + str(lr[4]) + " ##########")
        fail_per_lr += 1
        hangup()
        exit()
    ...
    validation_result, error_list = validation(final_list, lr[4], arg, error_list)
    ...
    if fail_per_lr == 0:
        ...
        print("\n\n#################################################################")
        print("\nOver All Test Result for " + str(lr[4]) + " GS Test : PASS\n")
        print("#################################################################")
        exit(0)
    else:
        print("#################################################################")
        print("Over All Test Result for " + str(lr[4]) + " GS Test : FAIL\n")
        print("#################################################################")
        exit(1)
