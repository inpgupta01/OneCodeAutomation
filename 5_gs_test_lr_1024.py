...
def main_function(arg):
    retrieveList(arg)
    error_list = []
    print("###########TEST CASE STARTED FOR LR " + str(lr[2]) + "############################\n\n")
    fail_per_lr = 0
    # Connecting calls on GS

    print("#####################Connecting calls on GS############## \n ")
    for ep in final_list:
        print("Endpoint " + str(ep) + " is connecting to VMR " + str(int(vmr[2])) + " at line rate " + str(
            int(lr[2])) + " kbps.")
        calling(ep, int(vmr[2]), int(lr[2]))
        time.sleep(3)
    ...
    if connection_status == 0:
        ...
        print("Not all EPs successfully connected to conference..Check logs for Eps Disconnection cause Result: ["
              "FAIL] ")
        print("##########Test Case failed for LR " + str(lr[2]) + " ##########")
        fail_per_lr += 1
        hangup()
        exit()
    ...
    validation_result, error_list = validation(final_list, lr[2], arg, error_list)
    ...
    if fail_per_lr == 0:
        ...
        print("#################################################################")
        print("Over All Test Result for " + str(lr[2]) + " GS Test : PASS\n")
        print("#################################################################")
        exit(0)
    else:
        print("#################################################################")
        print("Over All Test Result for " + str(lr[2]) + " GS Test : FAIL\n")
        print("#################################################################")
        exit(1)
