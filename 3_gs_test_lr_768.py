...
def main_function(arg):
    ...
    print("###########TEST CASE STARTED FOR LR " + str(lr[3]) + "############################\n\n")
    fail_per_lr = 0
    # Connecting calls on GS

    print("#####################Connecting calls on GS############## \n ")
    for ep in final_list:
        print("Endpoint " + str(ep) + " is connecting to VMR " + str(int(vmr[3])) + " at line rate " + str(
            int(lr[3])) + " kbps.")
        calling(ep, int(vmr[3]), int(lr[3]))
        time.sleep(3)
    ...
    if connection_status == 0:
        ...
        print("Not all EPs successfully connected to conference..Check logs for Eps Disconnection cause Result: ["
              "FAIL] ")
        print("##########Test Case failed for LR " + str(lr[3]) + " ##########")
        fail_per_lr += 1
        hangup()
        exit()
    ...
    validation_result, error_list = validation(final_list, lr[3], arg, error_list)
